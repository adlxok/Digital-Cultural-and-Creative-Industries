...
public class CommunityCommentController implements CommunityConstant {
    ...
    public CommonResult commentList(@RequestParam(value = "pageNum", defaultValue = DEFAULT_PAGENUM) Integer pageNum,
        ...
        for (CommunityComment communityComment : list) {
            ...
            //顶级评论的作者
            User user = userService.queryUserByIdInCache(communityComment.getUserId());
            postComInfo.put("commentUser", user);
    ...
    @ApiOperation("回复评论列表")
    @RequestMapping(value = "/replyCommentList/{replyId}", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult replyCommentList(@PathVariable Integer replyId) {
        List<CommunityComment> list = communityCommentService.queryReplyCommentList(replyId);
        if (list.isEmpty()) {
            return CommonResult.success("该评论没有回复");
        }
            List<Map<String, Object>> replylist = new ArrayList<>();
            for (CommunityComment communityComment : list) {
                Map<String, Object> map = new HashMap<>();
                map.put("comment", communityComment);
                User user = userService.queryUserByIdInCache(communityComment.getUserId());
                map.put("user", user);
                User target = userService.queryUserByIdInCache(communityComment.getTargetId());
